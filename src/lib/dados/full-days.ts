 ...
 * decide férias precisa saber quem mais do mesmo time está fora, porque duas
 ...
 *
 * O ALERTA DE FÉRIAS VENCENDO é risco trabalhista, não organização: passados
 * 12 meses da admissão (ou das últimas férias) sem o descanso, a empresa passa
 * a dever em dobro. Por isso ele não é mais uma coluna da tabela — aparece em
 * destaque próprio.
 */
export type LinhaDoRelatorio = PessoaDoTime & {
  ...
  ausencias: number;
  licencas: number;
  /** Null quando a pessoa nunca tirou férias: aí a conta é desde a admissão. */
...
export async function relatorioDoPeriodo(
  ...
  return time.map((pessoa) => {
    ...
    // Tirada é a que já começou; agendada é a aprovada que ainda vem. A régua
    // é a data de início comparada com hoje, e não o status: uma férias
    // aprovada em janeiro para dezembro não foi "tirada".
