...
export async function solicitar(dados: unknown): Promise<Resultado<string>> {
  return executarAcao("solicitar", async () => {
    ...
    // Os triggers de férias devolvem a mensagem já escrita para gente ler —
...
async function avisarOsSocios(
  ...
  try {
    ...
    const rotulo = tipo === "ferias" ? "férias" : tipo === "licenca" ? "licença" : "ausência";
 ...
 * `team_presence_proteger` — sem ele, um clique apagaria as férias de alguém e
