...
async function Conteudo({ usuarioId, clienteId }: { usuarioId: string; clienteId: string }) {
  ...
  return <ListaDeAprovacoes esperando={esperando} decididas={decididas} somenteLeitura />;
}
...
export default async function AprovacoesDoClienteVistasPelaEquipe({
  ...
  const [sessao, cliente] = await Promise.all([exigirEquipe(), obterClientePeloSlug(slug)]);
