...
-- --- O Resumo Semanal e privado --------------------------------------------
--
-- O registro e da pessoa, e o Sprint 7 vai usa-lo na conversa de
-- desenvolvimento individual. Nem a socia le o de outro -- se um dia a agencia
-- quiser o contrario, que seja uma decisao explicita, com policy nova e aviso
-- na tela, e nao um descuido que ninguem percebeu.

delete from public.weekly_entries;

select teste.cenario('Bruno registra a entrega da semana dele',
  '44444444-4444-4444-4444-444444444444',
  format($fmt$
    insert into public.weekly_entries (user_id, data, descricao)
    values (%L, current_date, %L)
  $fmt$, '44444444-4444-4444-4444-444444444444', 'Fechei a arte da campanha'), 'ok', 1);

select teste.cenario('Bruno le o proprio registro',
  '44444444-4444-4444-4444-444444444444',
  'select 1 from public.weekly_entries', 'ok', 1);

select teste.cenario('A SOCIA nao le o registro do Bruno', :ANA,
  'select 1 from public.weekly_entries', 'ok', 0);

select teste.cenario('O desenvolvedor tambem nao le', :DIEGO,
  'select 1 from public.weekly_entries', 'ok', 0);

select teste.cenario('Ninguem escreve entrega no nome de outra pessoa', :ANA,
  format($fmt$
    insert into public.weekly_entries (user_id, data, descricao)
    values (%L, current_date, %L)
  $fmt$, '44444444-4444-4444-4444-444444444444', 'Entrega que a Ana inventou'), 'recusa');

select teste.cenario('A socia nao apaga o registro do Bruno', :ANA,
  'delete from public.weekly_entries', 'recusa');

select teste.cenario('O cliente nao alcanca a tabela', :JOANA,
  format($fmt$
    insert into public.weekly_entries (user_id, data, descricao)
    values (%L, current_date, %L)
  $fmt$, :JOANA, 'Nao deveria entrar'), 'recusa');
